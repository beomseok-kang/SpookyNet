...
class SpookyNet(nn.Module):
    ...
    def _atomic_properties_dynamic_debug(
        self,
        N: int,
        Q: torch.Tensor,
        S: torch.Tensor,
    ...
    def atomic_properties_debug(
        ...
        return self._atomic_properties_dynamic_debug(
            N=N,
            Q=Q,
            S=S,
            Z=Z,
    ...
    def energy_debug(
        ...
        if batch_seg is None:  # assume a single batch
            batch_seg = Z.new_zeros(Z.size(0))
        (f, ea, qa, ea_rep, ea_ele, ea_vdw, pa, c6) = self.atomic_properties_debug(
            Z=Z,
            Q=Q,
            S=S,
            R=R,
    ...
    def forward(
        ...
        if use_forces:
            ...
            if debug:
                energy_fn = self.energy_debug
            
            (energy, f, ea, qa, ea_rep, ea_ele, ea_vdw, pa, c6) = energy_fn(
                Z=Z,
                Q=Q,
                S=S,
                R=R,
